...
def rm2svg(input_file, coloured_annotations=False):
    max_x = 0
    max_y = 0
    ...
    output += '<g id="p1" style="display:inline"><filter id="blurMe"><feGaussianBlur in="SourceGraphic" stdDeviation="10" /></filter>'
    ...
    ### EXTERMELY IMPORTANT THE BELOW RESETS ###
    ...
    ### EXTERMELY IMPORTANT THE ABOVE RESETS ###
    ...
    for layer in range(nlayers):
        ...
        for stroke in range(nstrokes):
            ...
            for segment in range(nsegments):
                fmt = '<ffffff'
                xpos, ypos, speed, tilt, width, pressure = struct.unpack_from(fmt, data, offset); offset += struct.calcsize(fmt)
                ratio = (max_y/max_x)/(1872/1404)
                if ratio > 1:
                    xpos = ratio*((xpos*max_x)/1404)
                    ypos = (ypos*max_y)/1872
                else:
                    xpos = (xpos*max_x)/1404
                    ypos = (1/ratio)*(ypos*max_y)/1872
    ...
    output += '<rect x="0" y="0" width="{}" height="{}" fill-opacity="0"/>'.format(max_x, max_y)
    output += '</g>' # Closing page group
    output += '</svg>' # END notebook
    
    specs = '<svg xmlns="http://www.w3.org/2000/svg" height="{}" width="{}">'.format(max_y, max_x) # BEGIN Notebook
    specs += '''
...
class Pencil(Pen):
    ...
    def get_segment_width(self, speed, tilt, width, pressure, last_width):
        ...
        mamax_x = self.base_width * 10
        segment_width = segment_width if segment_width < mamax_x else mamax_x
        return segment_width
